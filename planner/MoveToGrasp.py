...
class MoveToGrasp(Action):
    """
    Navigates fro PR2 initialization to the pre-grasp pose in front of the object of interest
    """
    ...
    def state_init(self):
        ...
        if PRESEEDED_IK and FLOOR_FOR_BRICKS == 2:
            self.q_goal = theta_adjusted_pos(np.array([ 3.04807673e+00,  6.61922066e-01, -6.31666468e+00,  3.10000000e-01,
            4.20006077e-08, -3.19254566e-07, -1.22000057e+00, -1.64782089e-01,
            -1.61486587e+00, -1.25638647e+00,  8.63177944e-01,  0.00000000e+00,
            8.63178236e-01,  5.48000000e-01,  1.21999968e+00,  2.00000109e-01,
            2.52000022e+00, -1.37999935e+00 ,-1.78869323e-07,  0.00000000e+00,
            -3.72520746e-04,  1.22174095e-01]))
        else:
    ...
    def run(self, prev_command: Command):
        ...
        if self.rrt_ix + 1 < len(self.path) and matching_q_no_fingers(self.continuous_state[:self.pr2.num_joints()], self.path[self.rrt_ix], atol=1e-2):
            self.rrt_ix += 1
            self.pr2 = PR2(self.playground.construct_welded_sim(self.continuous_state))
            plant_context = self.pr2.get_fresh_plant_context()
        ...
        if matching_q_no_fingers(self.continuous_state[:self.pr2.num_joints()], self.path[-1], atol=8e-2):
            done = True
        
        return Command(self.pr2, self.traj_opt.value(self.toppra_path.value(t))), done

    def init_traj_opt(self, q_start, q_end, min_t=0.5, max_t=50.0, n_ctrl_pts=10, eps=1e-2, avoid_collisions=True, start=False, end=False):
        plant_context = self.pr2.get_fresh_plant_context()
        solver = SnoptSolver()
        ...
        traj_opt.AddPositionBounds(
            ...
            self.pr2.plant.GetPositionUpperLimits()[:num_q]
        )
...
class PR2_Problem(Problem):
    def __init__(self, pr2, query_object, plant_context, q_start: np.array, q_goal: np.array):
        ...
        for i in range(lb.shape[0]):
            if i == 2:
                range_list.append(Range(0, 2*np.pi, wrap_around=True))
            else:
